...
public class Scrapper {
    ...
    /**
     * input a value into the text box identified by the elementId; the
     * value is placed in the value attribute
     * @param elementId
     * @param text
     * @return
     */
    ...
    /**
     * input a value into the text box identified by the XPath; the
     * value is placed in the value attribute
     * @param xpath
     * @param text
     * @return
    ...
    public Scrapper visitAnchor(String elementId, long waitToLoad) throws Exception {
        String url = ((HtmlAnchor)this.htmlPage.getElementById(elementId)).getHrefAttribute().toString();
        return this._visitAnchor(url,waitToLoad);
    }

    public Scrapper visitAnchorX(String xpath, long waitToLoad) throws Exception {
        String url = ((HtmlAnchor)this.htmlPage.getFirstByXPath(xpath)).getHrefAttribute().toString();
       return this._visitAnchor(url,waitToLoad);

    }


    private Scrapper _visitAnchor(String url, long waitToLoad) throws Exception {
        try {
            URI uri = new URI(url);
            if (!uri.isAbsolute()) {
                url = this.htmlPage.getBaseURL() + url;
            }
            return this.visit(url,waitToLoad);
        } catch (URISyntaxException urie) {
            throw new Exception(urie.getMessage(),urie);
        }
    }
